...
- 事業内容: 「その業務で実際にやること（具体的な作業・業務範囲）」を簡潔に記述する。
  公告本文や仕様書・公募要領の抜粋に示された作業内容・対象・成果物を読み取り、
  1〜3文で「何をする業務か」が一読で分かるようにまとめる（例:「○○の設計・施工を行う」
  「△△システムの保守・運用を行う」「□□に関する調査・分析と報告書作成を行う」）。
  タイトルをそのまま繰り返すだけにしない。冗長な前置きや背景説明は省き、やることに絞る。
