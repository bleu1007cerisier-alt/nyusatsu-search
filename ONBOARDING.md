...
GitHub Actions（毎日 9:30/13:30/15:30 JST, cron）
  ...
  → NEDOを巡回し dataset/tenders.csv に追記マージ（消さず蓄積。概要・予算・決定事業者も取得）
  → 変更があれば自動コミット&push
  → Render が自動再デプロイ
...
- 予算規模(amount): 公募詳細ページ本文の「予算規模：」を抽出し**万円表記に統一**（`_format_amount`）。本文に無い案件は空（多くの公募は予算が**公募要領PDF**にのみ記載）。
- 決定事業者(awardee): 結果ページの「実施予定先」直後の社名を抽出（HTMLに社名がある案件のみ。多くは添付PDFのみで取得不可）。
...
2. **仕様書/公募要領PDFからの予算抽出**: 本文に予算が無い公募が多い。PDFは技術的に解析可能だが表現がバラバラで精度低・処理重（pypdfで検証済み・未実装）。やるなら腰を据えて。
